...
extern int getloadavg (double __loadavg[], int __nelem)
  __attribute__ ((__nothrow__, __leaf__)) __attribute__ ((__nonnull__ (1)));
void
differenceInteger (struct integer a, struct integer b, struct integer *c)
{
  int i, t;
  c->length = ((a.length) > (b.length) ? (a.length) : (b.length)) + 1;
  ...
  for (i = 0; i < c->length; i++)
	...
	t +
	((i <
	  a.length ? a.integer[i] : (a.length ==
				     0 ? 0 : (a.integer[a.length - 1] >=
					      0 ? 0 : -1))) & 0x00ff) + ((i <
									  b.
									  length
									  ?
  ...
  if (c->length > 0)
      ...
      if (t == -1 || t == 0)
	{
	  ...
	  i++;
	  c->size =
	    t == -1 ? i : i == 0 ? i : c->integer[i - 1] > 0 ? i : i + 1;
	}
      else
	c->size = c->length;
    }
...
void
differenceEqualsInteger (struct integer *a, struct integer b)
{
  int i, t;
  t = 1;
	...
	((i <
	  a->length ? a->integer[i] : (a->length ==
				       0 ? 0 : (a->integer[a->length - 1] >=
						0 ? 0 : -1))) & 0x00ff) +
	((i <
	  b.length ? ~b.integer[i] : (b.length ==
				      0 ? -1 : (b.integer[b.length - 1] >=
						0 ? -1 : 0))) & 0x00ff);
  ...
  if (a->length > 0)
    {
      t = a->integer[i = (a->length - 1)];
      if (t == -1 || t == 0)
	{
	  for (i = a->length - 2; i >= 0 && a->integer[i] == t; i--);
	  i++;
	  a->size =
	    t == -1 ? i : i == 0 ? i : a->integer[i - 1] > 0 ? i : i + 1;
	}
      else
	a->size = a->length;
    }
