...
extern int getloadavg (double __loadavg[], int __nelem)
  __attribute__ ((__nothrow__, __leaf__)) __attribute__ ((__nonnull__ (1)));
struct integer *
readInteger (char *str)
{
  struct integer *num;
  struct decimal dec;
  int s, i, j, c;
  long t;
  num = (struct integer *) malloc (sizeof (struct integer));
  s = 0;
  while (str[s] != 0)
    s++;
  if (str[0] == '-')
    s--;
  dec.length = (s + 8) / 9;
  dec.integer = (int *) malloc (dec.length * sizeof (int));
  for (i = 0; i < dec.length; i++)
    dec.integer[i] = 0;
  if (dec.length > 0 && str[0] == '-')
    {
      dec.integer[dec.length - 1] = 0xC0000000;
      str = str + 1;
    }
  for (i = 0; i < s; i++)
    {
      t = dec.integer[i / 9] & 0x3fffffff;
      t += (str[s - i - 1] - '0') * power[i % 9];
      dec.integer[i / 9] &= 0xc0000000;
      dec.integer[i / 9] |= t;
    }
  ...
  if (dec.length && dec.integer[dec.length - 1] < 0)
	...
	{
	  t += (~num->integer[i] & 0x00ff);
	  num->integer[i] = t & 0x00ff;
	  t >>= 8;
	}
    }
  free (dec.integer);
