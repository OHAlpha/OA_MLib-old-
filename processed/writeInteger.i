     ...
     void writeInteger (char *str, struct integer arr)
{
  ...
  char *a, *s;
  int r, c, i, j;
  s = (char *) malloc ((arr.length * 8 / 3 + 3) * sizeof (char *));
  a = (char *) malloc (arr.length * sizeof (char *));
  ...
  if (as < 0)
    s[--j] = '-';
  for (i = j; i <= (arr.length * 8 / 3 + 3); i++)
    str[i - j] = s[i];
}

char *
strInteger (struct integer arr)
{
  char as;
  char *a, *s;
  int r, c, i, j;
...
void
printInteger (struct integer arr)
{
  char as;
  char *a, *s;
  int r, c, i, j;
