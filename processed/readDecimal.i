...
struct decimal *
readDecimal (char *str)
{
  struct decimal *num;
  num = (struct decimal *) malloc (sizeof (struct decimal));
  int s, i, t;
  s = 0;
  while (str[s] != 0)
    s++;
  if (str[0] == '-')
    s--;
  num->length = (s + 8) / 9;
  num->integer = (int *) malloc (num->length * sizeof (int));
  for (i = 0; i < num->length; i++)
    num->integer[i] = 0;
  if (num->length > 0 && str[0] == '-')
    {
      num->integer[num->length - 1] = 0xC0000000;
      str = str + 1;
    }
  for (i = 0; i < s; i++)
    {
      t = num->integer[i / 9] & 0x3fffffff;
      t += (str[s - i - 1] - '0') * power[i % 9];
      num->integer[i / 9] &= 0xc0000000;
      num->integer[i / 9] |= t;
    }
  return num;
}
